...
func multiClusterAppUpgrade(ctx *cli.Context) error {
	...
	answers, _, err = processFileInputs(ctx, c, nil, answers, "", false)
	if err != nil {
		return err
	}
...
func multiClusterAppTemplateInstall(ctx *cli.Context) error {
	...
	answers, _, err := processFileInputs(ctx, c, templateVersion, nil, "", interactive)
	if err != nil {
		return err
	}
...
func getTargetInput(ctx *cli.Context, c *cliclient.MasterClient) (*managementClient.UpdateMultiClusterAppTargetsInput, error) {
	...
	answers, _, err := processFileInputs(ctx, c, nil, nil, "", false)
	if err != nil {
		return nil, err
	}
